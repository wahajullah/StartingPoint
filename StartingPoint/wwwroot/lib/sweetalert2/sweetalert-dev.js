...
var _defaultParStartingPoint = require('./modules/default-parStartingPoint');

var _defaultParStartingPoint2 = _interopRequireWildcard(_defaultParStartingPoint);

var _setParameters = require('./modules/set-parStartingPoint');
...
exports['default'] = sweetAlert = swal = function () {
  ...
  /*
   * Use argument if defined or default value from parStartingPoint object otherwise.
   * Supports the case where a default value is boolean true and should be
   * overridden by a corresponding explicit argument which is boolean false.
   */
  function argumentOrDefault(key) {
    var args = customizations;
    return args[key] === undefined ? _defaultParStartingPoint2['default'][key] : args[key];
  }
  ...
  var parStartingPoint = _extend$hexToRgb$isIE8$logStr$colorLuminance.extend({}, _defaultParStartingPoint2['default']);

  switch (typeof customizations) {

    // Ex: swal("Hello", "Just testing", "info");
    case 'string':
      parStartingPoint.title = customizations;
      parStartingPoint.text = arguments[1] || '';
      parStartingPoint.type = arguments[2] || '';
      break;
    ...
    case 'object':
      ...
      parStartingPoint.title = customizations.title;

      for (var customName in _defaultParStartingPoint2['default']) {
        parStartingPoint[customName] = argumentOrDefault(customName);
      }

      // Show "Confirm" instead of "OK" if cancel button is visible
      parStartingPoint.confirmButtonText = parStartingPoint.showCancelButton ? 'Confirm' : _defaultParStartingPoint2['default'].confirmButtonText;
      parStartingPoint.confirmButtonText = argumentOrDefault('confirmButtonText');

      // Callback function when clicking on "OK"/"Cancel"
      parStartingPoint.doneFunction = arguments[1] || null;
  ...
  _setParameters2['default'](parStartingPoint);
  ...
  var onButtonEvent = function onButtonEvent(e) {
    return _handleButton$handleConfirm$handleCancel.handleButton(e, parStartingPoint, modal);
  };
  ...
  var onKeyEvent = function onKeyEvent(e) {
    return _handleKeyDown2['default'](e, parStartingPoint, modal);
  };
...
/*
 * Set default parStartingPoint for each popup
 * @param {Object} userParStartingPoint
 */
sweetAlert.setDefaults = swal.setDefaults = function (userParStartingPoint) {
  if (!userParStartingPoint) {
    throw new Error('userParStartingPoint is required');
  }
  if (typeof userParStartingPoint !== 'object') {
    throw new Error('userParStartingPoint has to be a object');
  }

  _extend$hexToRgb$isIE8$logStr$colorLuminance.extend(_defaultParStartingPoint2['default'], userParStartingPoint);
};
...
},{"./modules/default-parStartingPoint":2,"./modules/handle-click":3,"./modules/handle-dom":4,"./modules/handle-key":5,"./modules/handle-swal-dom":6,"./modules/set-parStartingPoint":8,"./modules/utils":9}],2:[function(require,module,exports){
'use strict';

Object.defineProperty(exports, '__esModule', {
  value: true
});
var defaultParStartingPoint = {
  title: '',
  text: '',
  type: null,
...
exports['default'] = defaultParStartingPoint;
module.exports = exports['default'];
 ...
 */
var handleButton = function handleButton(event, parStartingPoint, modal) {
  ...
  var doneFunctionExists = parStartingPoint.doneFunction && modal.getAttribute('data-has-done-function') === 'true';

  // Since the user can change the background-color of the confirm button programmatically,
  // we must calculate what the color should be on hover/active
  var normalColor, hoverColor, activeColor;
  if (targetedConfirm && parStartingPoint.confirmButtonColor) {
    normalColor = parStartingPoint.confirmButtonColor;
  ...
  switch (e.type) {
    ...
    case 'click':
      ...
      if (!clickedOnModal && !clickedOnModalChild && modalIsVisible && !parStartingPoint.allowOutsideClick) {
        break;
      }

      if (targetedConfirm && doneFunctionExists && modalIsVisible) {
        handleConfirm(modal, parStartingPoint);
      } else if (doneFunctionExists && modalIsVisible || targetedOverlay) {
        handleCancel(modal, parStartingPoint);
 ...
 */
var handleConfirm = function handleConfirm(modal, parStartingPoint) {
  ...
  parStartingPoint.doneFunction(callbackValue);

  if (parStartingPoint.closeOnConfirm) {
    sweetAlert.close();
  }
  // Disable cancel and confirm button if the parameter is true
  if (parStartingPoint.showLoaderOnConfirm) {
 ...
 */
var handleCancel = function handleCancel(modal, parStartingPoint) {
  // Check if callback function expects a parameter (to track cancel actions)
  var functionAsStr = String(parStartingPoint.doneFunction).replace(/\s/g, '');
  var functionHandlesCancel = functionAsStr.substring(0, 9) === 'function(' && functionAsStr.substring(9, 10) !== ')';

  if (functionHandlesCancel) {
    parStartingPoint.doneFunction(false);
  }

  if (parStartingPoint.closeOnCancel) {
    sweetAlert.close();
  }
};
...
var handleKeyDown = function handleKeyDown(event, parStartingPoint, modal) {
  var e = event || window.event;
  ...
  if (keyCode === 9) {
    ...
    if (parStartingPoint.confirmButtonColor) {
      _setFocusStyle.setFocusStyle($targetElement, parStartingPoint.confirmButtonColor);
    }
  } else {
    if (keyCode === 13) {
    ...
    } else if (keyCode === 27 && parStartingPoint.allowEscapeKey === true) {
...
var _defaultParStartingPoint = require('./default-parStartingPoint');
...
var resetInput = function resetInput() {
  ...
  $input.value = _defaultParStartingPoint2['default'].inputValue;
  $input.setAttribute('type', _defaultParStartingPoint2['default'].inputType);
  $input.setAttribute('placeholder', _defaultParStartingPoint2['default'].inputPlaceholder);
...
},{"./default-parStartingPoint":2,"./handle-dom":4,"./injected-html":7,"./utils":9}],7:[function(require,module,exports){
"use strict";
 ...
 */
var setParameters = function setParameters(parStartingPoint) {
   ...
   * Title
   */
  $title.innerHTML = parStartingPoint.html ? parStartingPoint.title : _hasClass$addClass$removeClass$escapeHtml$_show$show$_hide$hide.escapeHtml(parStartingPoint.title).split('\n').join('<br>');

  /*
   * Text
   */
  $text.innerHTML = parStartingPoint.html ? parStartingPoint.text : _hasClass$addClass$removeClass$escapeHtml$_show$show$_hide$hide.escapeHtml(parStartingPoint.text || '').split('\n').join('<br>');
  if (parStartingPoint.text) _hasClass$addClass$removeClass$escapeHtml$_show$show$_hide$hide.show($text);

  /*
   * Custom class
   */
  if (parStartingPoint.customClass) {
    _hasClass$addClass$removeClass$escapeHtml$_show$show$_hide$hide.addClass(modal, parStartingPoint.customClass);
    modal.setAttribute('data-custom-class', parStartingPoint.customClass);
  } else {
  ...
  if (parStartingPoint.type && !_isIE8.isIE8()) {
    var _ret = (function () {

      var validType = false;

      for (var i = 0; i < alertTypes.length; i++) {
        if (parStartingPoint.type === alertTypes[i]) {
          validType = true;
          break;
        }
      }
      ...
      if (!validType) {
        logStr('Unknown alert type: ' + parStartingPoint.type);
        return {
          v: false
        };
      }
      ...
      if (typesWithIcons.indexOf(parStartingPoint.type) !== -1) {
        $icon = modal.querySelector('.sa-icon.' + 'sa-' + parStartingPoint.type);
      ...
      switch (parStartingPoint.type) {
        ...
        case 'input':
        case 'prompt':
          $input.setAttribute('type', parStartingPoint.inputType);
          $input.value = parStartingPoint.inputValue;
          $input.setAttribute('placeholder', parStartingPoint.inputPlaceholder);
   ...
   * Custom image
   */
  if (parStartingPoint.imageUrl) {
    var $customIcon = modal.querySelector('.sa-icon.sa-custom');

    $customIcon.style.backgroundImage = 'url(' + parStartingPoint.imageUrl + ')';
    _hasClass$addClass$removeClass$escapeHtml$_show$show$_hide$hide.show($customIcon);

    var _imgWidth = 80;
    var _imgHeight = 80;

    if (parStartingPoint.imageSize) {
      var dimensions = parStartingPoint.imageSize.toString().split('x');
      var imgWidth = dimensions[0];
      var imgHeight = dimensions[1];

      if (!imgWidth || !imgHeight) {
        logStr('Parameter imageSize expects value with format WIDTHxHEIGHT, got ' + parStartingPoint.imageSize);
      } else {
        _imgWidth = imgWidth;
   ...
   * Show cancel button?
   */
  modal.setAttribute('data-has-cancel-button', parStartingPoint.showCancelButton);
  if (parStartingPoint.showCancelButton) {
   ...
   * Show confirm button?
   */
  modal.setAttribute('data-has-confirm-button', parStartingPoint.showConfirmButton);
  if (parStartingPoint.showConfirmButton) {
   ...
   */
  if (parStartingPoint.cancelButtonText) {
    $cancelBtn.innerHTML = _hasClass$addClass$removeClass$escapeHtml$_show$show$_hide$hide.escapeHtml(parStartingPoint.cancelButtonText);
  }
  if (parStartingPoint.confirmButtonText) {
    $confirmBtn.innerHTML = _hasClass$addClass$removeClass$escapeHtml$_show$show$_hide$hide.escapeHtml(parStartingPoint.confirmButtonText);
  }
  ...
  if (parStartingPoint.confirmButtonColor) {
    ...
    $confirmBtn.style.backgroundColor = parStartingPoint.confirmButtonColor;

    // Set the confirm button color to the loading ring
    $confirmBtn.style.borderLeftColor = parStartingPoint.confirmLoadingButtonColor;
    $confirmBtn.style.borderRightColor = parStartingPoint.confirmLoadingButtonColor;

    // Set box-shadow to default focused button
    _getModal$getInput$setFocusStyle.setFocusStyle($confirmBtn, parStartingPoint.confirmButtonColor);
  }

  /*
   * Allow outside click
   */
  modal.setAttribute('data-allow-outside-click', parStartingPoint.allowOutsideClick);

  /*
   * Callback function
   */
  var hasDoneFunction = parStartingPoint.doneFunction ? true : false;
  modal.setAttribute('data-has-done-function', hasDoneFunction);

  /*
   * Animation
   */
  if (!parStartingPoint.animation) {
    modal.setAttribute('data-animation', 'none');
  } else if (typeof parStartingPoint.animation === 'string') {
    modal.setAttribute('data-animation', parStartingPoint.animation); // Custom animation
  } else {
   ...
   * Timer
   */
  modal.setAttribute('data-timer', parStartingPoint.timer);
};
...
/*
 * Allow user to pass their own parStartingPoint
 */
